...
def quad_sieve(n, b) :
    ...
    print('ARRAY arr[i] OF NUMBERS NEAREST TO SQUARE ROOT(N) :',a)
    print("THE VALUE OF arr[i] ^ 2 - N:",q)
    ...
    for i in range(len(q)) :
        ...
        for j in range(1, len(factor_base)) :
            ...
            vec[j] = count
        exp_vec.append(vec)
    print('FACTOR BASE EXPONENT VECTOR : ')
    for row in exp_vec :
        print(row)
    ...
    x = 1
    y2 = 1
    print('SELECTEDN EXPONENT VECTORS : ')
    for i in range(len(lc_list)) :
        if lc_list[i] == 1 :
            print('arr[i] = ' + str(a[i]) + '  arr[i]^2-N = ' + str(q[i]),)
            x *= a[i]
            y2 *= q[i]
    y = int(math.sqrt(y2))
    x = x % n
    ...
    g = int(gcd(x + y, n))
    if f ==1 and g==1:
        print("NO FACTORS, HENCE THE NUMBER IS PRIME.")
